...
--
-- GOVERNED CHANGE: finance_payroll_sod_policy rows are append-only and versioned
--   (draft -> pending_approval -> active, superseding the prior active row). The
--   proposer/approver separation and workflow routing are enforced by the payroll
--   SoD service + the central workflow engine; the guards here are the last line of
--   defence (status transitions, maker != checker, single active row).
--
-- The two RPC bodies below are reproduced VERBATIM from
-- 20260919000424_finance_payroll_certification_release_tx.sql, with ONLY the SoD
-- checks parameterised by v_run.sod_level (generated, not hand-copied).

-- ── Policy table (versioned, append-only) ────────────────────────────────────
...
-- ── Active-level accessor (used as the run column default) ───────────────────
...
begin
  ...
  -- SoD floor (every level): the preparer can never release their own run.
  if p_actor_id = v_run.created_by then
    raise exception 'finance_payroll_release: releaser must differ from the preparer'
      using errcode = 'PR403';
  end if;
  -- Level >= 3 additionally separates the approver from the releaser.
  ...
  -- Level 4 (strictest) additionally separates the certifier from the releaser.
...
begin
  ...
  -- SoD floor (every level): the preparer can never fund their own run.
  if p_actor_id = v_run.created_by then
    raise exception 'finance_payroll_funding: the preparer cannot confirm funding'
      using errcode = 'PR403';
  end if;
  -- Level >= 3 additionally separates the approver from the funder.
  if v_run.sod_level >= 3 and p_actor_id = v_run.approved_by then
  ...
  -- Level 4 (strictest) additionally separates the certifier from the funder.
  if v_run.sod_level >= 4 and p_actor_id = v_cert.certified_by then
