-- ============================================================================
-- Atomic Workflow Creation — Shape-A slice A4: employee-loan submit
-- (audit finding #3). Adds the `finance_employee_loans` branch to
-- public.workflow_submit_for_record_tx. create-or-replace with ALL FOUR branches
-- (finance_payroll_runs A1 + payroll_payslip_templates A2 + finance_remittances A3
-- + finance_employee_loans A4). Depends on 210/211/212/214/215 applied.
-- Operator-applied; idempotent. After applying:
--   NOTIFY pgrst, 'reload schema';
-- ============================================================================
-- A4 deltas: module finance_loan / type finance_loan_approval / trigger
-- finance.loan.submitted; from **draft OR rejected** -> pending_approval (per
-- sourceStatusMap.onStarted). A rejected loan resubmit carries a terminal prior
-- workflow (rejected/returned) -> supersedes, handled by the generic check.
-- Source UPDATE sets status + workflow_id only (no submitted_by column on loans).
-- Business event finance.loan.submitted (module/entity finance_loan/employee_loan),
-- audit action loan.submitted; NO handoff; first step role=finance_manager
-- (verified present in public.roles + the finance_loan binding). The wired
-- submitLoan adds the finance_manager assignee notification (previously provided
-- by startWorkflowForRecord's notifyTaskAssigned) via a post-RPC fan-out.
-- ============================================================================
...
begin
  ...
  -- Handoff intent (per-table). Payroll -> payroll_approval; template/remittance/loan -> none.
  if p_source_table = 'finance_payroll_runs' then
