-- ============================================================================
-- Finance Statutory — workflow source-transition receipt RPC (audit F3, P0)
-- Mirrors the payroll exemplar (mig 180). Depends on 150 (workflow_source_receipts).
-- Operator-applied; idempotent. After applying: NOTIFY pgrst, 'reload schema';
--   verify: select count(*)>0 from pg_proc where proname = 'finance_statutory_workflow_transition_tx';
--
-- Problem fixed: the outbox worker previously mutated the statutory version via
-- the ADAPTER callback (applyApprovedStatutoryVersion) BEFORE finalize; a
-- finalize retry re-ran the adapter against the already-approved row, which
-- correctly refuses (pending_approval gate) and dead-letters the transition.
-- This RPC is receipt-guarded exactly-once: a retry returns the stored result
-- and writes nothing. Rich post-commit effects (notification, config thread)
-- move to the worker handler afterCommit (parity with payroll).
-- ============================================================================
