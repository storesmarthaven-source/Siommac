-- ============================================================================
-- Finance Pay Components — workflow template + binding
-- ============================================================================
...
--
-- Single Finance Manager approval step (72 h SLA).
-- Segregation of duties (creator ≠ approver) is enforced in the adapter
-- (financePayComponentAdapter.ts) and the service layer, NOT in the template.
-- ============================================================================
...
begin
  ...
  select id into tpl_id from public.workflow_templates
    where template_key = 'finance_pay_component_approval';
  if tpl_id is null then
    insert into public.workflow_templates
      (template_key, module_key, workflow_type, name, description, status, is_active, current_version, definition)
    values
      ('finance_pay_component_approval', 'finance_pay_components', 'finance_pay_component_approval',
       'Finance Pay Component Approval',
       'Finance Manager approval of a pay-component create, update or retire request before the change is applied.',
       'active', true, 1, '{}'::jsonb)
    returning id into tpl_id;
  else
    update public.workflow_templates
      set module_key      = 'finance_pay_components',
          workflow_type   = 'finance_pay_component_approval',
          status          = 'active',
          current_version = 1
      where id = tpl_id;
  end if;
  ...
  delete from public.module_workflow_bindings
    where module_key    = 'finance_pay_components'
      and workflow_type  = 'finance_pay_component_approval'
      and trigger_event  = 'finance.payroll.component.change_submitted'
  ...
  values
    ('finance_pay_components', 'finance_pay_component_approval', 'finance.payroll.component.change_submitted',
