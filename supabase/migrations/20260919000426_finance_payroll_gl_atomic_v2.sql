  ...
  select md5(coalesce(
    jsonb_agg(
      jsonb_build_object(
        ...
        -- numeric(15,2) to match the stored journal-line columns exactly, so the
        -- release-preflight recompute (which reads those columns) produces the
        -- identical jsonb text and md5. A bare ::numeric drops the scale
        -- (6000 vs 6000.00) and the control checksum would never re-validate.
        'debit', (line.value->>'debit')::numeric(15,2),
        'credit', (line.value->>'credit')::numeric(15,2)
      )
