-- ============================================================================
-- Atomic Workflow Creation — Shape-A batch (finance/HR group): A6/A7/A8/A9
-- (audit finding #3). Adds FOUR branches to public.workflow_submit_for_record_tx:
--   finance_statutory_versions (A8) · hr_employee_pay_items (A9) ·
--   finance_disbursements (A6) · finance_expense_claims (A7).
-- create-or-replace with ALL branches (payroll A1 + payslip-template A2 +
-- remittance A3 + loan A4 + the four here). Depends on 210/211/212 applied.
-- All four use hr_audit_log + a STATIC role first step (no dynamic assignee, no
-- handoff). Operator-applied; idempotent. After applying:
--   NOTIFY pgrst, 'reload schema';
-- ============================================================================
-- Per-branch contract (verified against the create-table migrations):
--   A8 finance_statutory_versions: from draft -> pending_approval; module
--      finance_statutory / finance_statutory_approval / finance.statutory.version.submitted;
--      NO ref column (ref computed 'SV-'||upper(left(id,8))); owner created_by;
--      event finance.statutory.version.submitted / entity statutory_version /
--      audit statutory_version.submitted; role finance_manager.
--   A9 hr_employee_pay_items: from draft -> pending_approval; module hr_compensation /
--      hr_compensation_change_approval / hr.compensation.item.submitted; ref
--      coalesce(item_no,'PIT-'||upper(left(id,8))); owner created_by; entity pay_item /
--      audit pay_item.submitted; role hr_manager.
--   A6 finance_disbursements: from draft -> submitted; module finance_disbursements /
--      finance_disbursement_approval / finance.disbursement.submitted; ref
--      disbursement_no; owner created_by; entity disbursement / audit
--      disbursement.submitted; role finance_manager. Source UPDATE also merges
--      metadata.submittedAt = now() (atomic, replaces the legacy separate write).
--   A7 finance_expense_claims: from draft -> submitted; module finance_expenses /
--      finance_expense_approval / finance.expense.submitted; ref claim_no; **owner
--      claimant_id** (not created_by); entity expense_claim / audit expense.submitted;
--      role finance_manager. The missing-receipt notify/ticket/thread side-effects
--      stay in the TS caller post-commit (best-effort), NOT in this txn.
-- ============================================================================
...
begin
  ...
  if p_source_table = 'finance_payroll_runs' then
    ...
    v_to_status    := 'submitted';
    v_owner        := v_exp.claimant_id;   -- claim owner is the claimant, not created_by
  ...
  -- Business event + module audit (generic, per-table descriptors — all hr_audit_log).
