-- ============================================================================
-- Audit remediation (2026-07-16): findings 1, 2, 7 + table grants
--   F1 (P0): wf_internal._create_instance now PINS role/fixed_user assignees to
--            the template resolver value — a caller-supplied assignee that does
--            not match the template raises WF422 (closes the explicit-start
--            role-redirect injection). Dynamic assignment types are unchanged
--            (their resolution is context-driven by design).
--   F2 (P0): workflow_create_and_start_tx threads siteId/departmentId from the
--            HSE payloads into _create_instance (scope provenance); the TS call
--            sites now also pass site/department/recordData into the binding
--            resolver so scoped bindings can actually match.
--   F7:      data fix — close open tasks whose parent workflow is terminal
--            (the mig-397 preflight cancelled instances but left their tasks).
--   P1:      explicit service-role grants for the two new messaging tables.
-- Operator-applied; idempotent. After applying: NOTIFY pgrst, 'reload schema';
--   verify: select position('does not match the template role' in prosrc) > 0
...
-- ── F7 data fix: tasks orphaned by terminal (incl. preflight-cancelled) workflows ──
...
-- ── P1: deterministic grants for the new messaging tables ──
