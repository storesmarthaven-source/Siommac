-- ============================================================================
-- Workflow administration commands: atomic publish/delegate/reassign and a
-- durable, fenced cancellation transition.
-- ============================================================================
-- Backend-only RPCs. Each command locks the owning aggregate, applies the
-- business mutation, and writes its audit/event in one transaction. Cancellation
-- gates the workflow and commits an outbox job; the worker applies the module
-- source transition before workflow_finalize_cancel_tx closes the workflow.
-- ============================================================================
...
begin
  ...
  -- Read the parent id, then lock parent -> all children in deterministic order.
...
begin
  ...
  -- Scope client request keys to the authenticated actor. Two users may
  -- legitimately generate the same UUID; one must not reserve the other's key.
  ...
  if p_command = 'cancel' then
    ...
    -- Delegation/reassignment is administrative routing, not the task's terminal
    -- decision. workflow_decisions has a one-row-per-task backstop; consuming it
    -- here would make the later approve/return/reject impossible.
    insert into public.workflow_audit_log
...
-- Finalize a queued cancellation only after the worker has successfully applied
-- the module source transition. The lease token fences stale workers.
