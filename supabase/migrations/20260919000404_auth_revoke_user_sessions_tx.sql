-- ============================================================================
-- auth_revoke_user_sessions_tx -- atomic forced logout (superadmin console)
-- ============================================================================
-- Replaces the sequential app-layer writes in revokeUserSessions() + the
-- route-level log_() call: revocation marker + refresh-token purge + ONE
-- app_event + ONE activity_logs audit row commit together or roll back
-- together. A crash between the old separate calls could revoke the epoch but
-- keep refresh tokens (or vice versa) and lose the audit trail entirely.
--
-- Idempotency: the app_events.dedupe_key ('auth.session.revoked:<key>') is the
-- retry receipt (app_events_dedupe_uidx enforces it). Same key + same target
-- returns the ORIGINAL result with replay=true and writes nothing new; same
-- key + a DIFFERENT target raises AU409.
--
-- Errcodes (5-char, module-prefixed like WF4xx/MG4xx):
--   AU404 target user not found        AU409 idempotency key reused w/ different payload
--   AU422 invalid input / self-revoke
...
begin
  ...
  -- Retry receipt: an event with this dedupe key IS the committed prior run.
  ...
  -- Lock the target user row: serializes concurrent revokes of the same user
  -- and proves existence inside the transaction.
  ...
  -- Exactly ONE event; the unique dedupe index is the concurrency backstop
  -- (a same-key race that lost the row lock on a DIFFERENT target lands here).
  begin
  ...
  -- Exactly ONE audit row, owned here (the route no longer writes its own).
