-- ============================================================================
-- Payroll Exceptions & Approvals (§15.3) — work-queue read model: user-selectable
-- SORT. Adds p_sort to finance_payroll_findings_work_queue and reworks the keyset
-- cursor into a uniform (sort_a, sort_b, id) tuple so pagination stays a clean
-- row-value comparison regardless of the chosen order.
--
-- Supported sorts (all resolve to ASC (sort_a, sort_b, id)):
--   priority (default) — severity_rank, then newest-first within severity (legacy order)
--   pay_date           — soonest pay date first (nulls last), then severity
--   due_date           — soonest due first (nulls last), then newest-first
--   newest / oldest    — by finding/task creation time, then severity
--
-- The old 9-arg overload is DROPPED first so PostgREST resolves a single function.
-- ============================================================================
drop function if exists public.finance_payroll_findings_work_queue(
...
declare
  ...
  v_sort     text    := coalesce(nullif(btrim(p_sort), ''), 'priority');
  v_parts    text[];
  v_cur_a    bigint;
  v_cur_b    bigint;
  v_cur_id   text;
...
begin
  ...
  if v_sort not in ('priority','pay_date','due_date','newest','oldest') then
    raise exception 'work_queue: unknown sort %', v_sort using errcode = 'PR422';
  end if;
  ...
  if p_cursor is not null and btrim(p_cursor) <> '' then
    ...
    begin
      v_cur_a  := v_parts[1]::bigint;
      v_cur_b  := v_parts[2]::bigint;
      v_cur_id := v_parts[3];
  ...
  sorted as (
    -- uniform keyset tuple: (sort_a, sort_b, id) ASC. Sentinel 9223372036854775807
    -- (max bigint) pushes null pay/due dates to the end.
    select s.*,
      case v_sort
        when 'newest'   then s.neg_micros
        when 'oldest'   then (extract(epoch from s.created_at) * 1000000)::bigint
        when 'pay_date' then coalesce((extract(epoch from s.run_pay_date) * 1000000)::bigint, 9223372036854775807)
        when 'due_date' then coalesce((extract(epoch from s.due_at)       * 1000000)::bigint, 9223372036854775807)
        else s.severity_rank::bigint
      end as sort_a,
      case v_sort
        when 'due_date' then s.neg_micros
        when 'priority' then s.neg_micros
        else s.severity_rank::bigint
      end as sort_b
    from tabbed s
  ),
  page as (
    select * from sorted s
    where p_cursor is null or btrim(p_cursor) = ''
       or (s.sort_a, s.sort_b, s.id) > (v_cur_a, v_cur_b, v_cur_id)
    order by s.sort_a, s.sort_b, s.id
    limit v_limit + 1
  ),
  windowed as (
    select * from page order by sort_a, sort_b, id limit v_limit
  ),
  items as (
  ...
  next_cursor as (
    select case when (select count(*) from page) > v_limit
      then (
        select w.sort_a || '|' || w.sort_b || '|' || w.id
        from windowed w
        order by w.sort_a desc, w.sort_b desc, w.id desc
        limit 1
      )
      else null end as nc
  ),
...
revoke all on function public.finance_payroll_findings_work_queue(
  integer, text, text, text[], text[], text[], uuid[], text, text, text
