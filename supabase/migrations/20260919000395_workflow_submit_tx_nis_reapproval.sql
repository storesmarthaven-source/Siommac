-- ============================================================================
-- Shape-C C1 (finding #3): NIS re-approval atomized.
-- Extends the finance_statutory_versions branch of workflow_submit_for_record_tx
-- to accept from-status='approved' in addition to 'draft', enabling the
-- upsertNisClasses re-approval path to use the RPC atomically instead of
-- the legacy startWorkflowForRecord + compensating-rollback dance.
-- ONLY the finance_statutory_versions branch changes (3 surgical edits):
--   1. Status gate: 'draft' -> 'draft' OR 'approved'
--   2. Audit action: from-status-aware (reopened_by_edit vs submitted)
--   3. UPDATE: clears approved_by when re-opening an approved version
-- All 13 other branches are byte-identical to 20260919000380.
-- Depends on migs 210/211/212/380 applied. Operator-applied; idempotent.
-- After applying:
--   NOTIFY pgrst, 'reload schema';
--   verify: select position('approved' in prosrc)>0
--             from pg_proc where proname='workflow_submit_for_record_tx';
-- ============================================================================
