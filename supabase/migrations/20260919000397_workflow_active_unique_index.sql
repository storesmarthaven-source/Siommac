-- ============================================================================
-- Final Cutover: active-workflow partial unique index + 23505->WF409 guard
-- (finding #3, SUBMIT_TX_DESIGN.md sections 9 and 11).
--
-- Design rationale:
--   netlify/functions/lib/workflow/SUBMIT_TX_DESIGN.md  (sections 9, 11)
--   netlify/functions/lib/workflow/FINAL_CUTOVER_CONTRACT.md
--
-- Depends on migrations 20260919000210, 20260919000211, 20260919000219,
-- 20260919000395, 20260919000396 applied.
--
-- Operator-applied; idempotent (CREATE UNIQUE INDEX IF NOT EXISTS).
-- After applying:
--   NOTIFY pgrst, 'reload schema';
--   verify: select indexname from pg_indexes
--             where indexname = 'uq_wf_one_active_per_record';
--
-- Sections in this migration:
--   1. DUP PREFLIGHT: cancel older duplicate active workflows before the index.
--   2. UPDATE wf_internal._create_instance to catch unique_violation -> WF409.
--   3. CREATE partial unique index uq_wf_one_active_per_record.
-- ============================================================================

-- ============================================================================
-- SECTION 1: DUP PREFLIGHT
--
-- Detect existing active-workflow pairs that would violate the new index and
-- cancel all but the most recent (by started_at DESC, id DESC as tiebreak).
-- Never deletes rows. Tags cancelled rows with cancelled_reason in metadata.
--
-- Canonical terminal status set: completed, approved, returned, rejected,
-- cancelled, closed (matches index WHERE clause).
--
-- Onboarding discriminator: the pair (module_key='hr_onboarding',
-- workflow_type='onboarding_custom_approval') is excluded from dedup because
-- multiple custom approval actions on one onboarding case are legitimate.
-- See FINAL_CUTOVER_CONTRACT.md section 4.
-- ============================================================================
do $$
declare
...
begin
  for v_rec in
    ...
    having count(*) > 1
  loop
    -- Cancel all but the single most recent active instance for this triplet.
    with keeper as (
      select id
      from public.workflow_instances
      where module_key       = v_rec.module_key
        and workflow_type    = v_rec.workflow_type
        and source_record_id = v_rec.source_record_id
        and status not in ('completed','approved','returned','rejected','cancelled','closed')
    ...
    select count(*) into v_rows_fixed from cancelled;

    v_groups_fixed := v_groups_fixed + 1;
  end loop;

  if v_groups_fixed > 0 then
    raise notice 'preflight_dedup_mig_397: cancelled older duplicates in % source-record groups (% rows total)',
      v_groups_fixed, v_rows_fixed;
  else
    raise notice 'preflight_dedup_mig_397: no duplicate active workflows found — index creation is clean';
  end if;
end
$$;

-- ============================================================================
-- SECTION 2: UPDATE wf_internal._create_instance
--
-- Wraps the INSERT INTO workflow_instances in a sub-block exception handler so
-- that a unique_violation (SQLSTATE 23505) from the new index is translated to
-- a WF409 custom SQLSTATE. The rpcHttpError helper in service.ts maps WF409 to
-- HTTP 409 via the WF_SQLSTATE_HTTP table (no TypeScript change required).
--
-- This is a full CREATE OR REPLACE of the function last defined in migration
-- 20260919000219. The only change from that version is the sub-block around
-- the INSERT INTO workflow_instances. All other logic is byte-identical.
-- ============================================================================
...
begin
  ...
  -- Sub-block: catch unique_violation (SQLSTATE 23505) from the partial unique
  -- index uq_wf_one_active_per_record and translate it to WF409 so the callers
  -- rpcHttpError helper maps it to HTTP 409.
  begin
...
-- ============================================================================
-- SECTION 3: PARTIAL UNIQUE INDEX
--
-- Enforces at most one active workflow per (module_key, workflow_type,
-- source_record_id) triplet.
--
-- Terminal status set: completed, approved, returned, rejected, cancelled, closed.
-- (Design doc SUBMIT_TX_DESIGN.md section 9.)
--
-- Onboarding discriminator: the pair ('hr_onboarding','onboarding_custom_approval')
-- is excluded because onboarding custom actions legitimately start multiple
-- concurrent approval workflows on the same case record (E3 in the design doc).
-- (Design doc SUBMIT_TX_DESIGN.md section 9 / FINAL_CUTOVER_CONTRACT.md section 4.)
-- ============================================================================
create unique index if not exists uq_wf_one_active_per_record
  on public.workflow_instances (module_key, workflow_type, source_record_id)
  where status not in ('completed','approved','returned','rejected','cancelled','closed')
    and not (module_key = 'hr_onboarding'
             and workflow_type = 'onboarding_custom_approval');
