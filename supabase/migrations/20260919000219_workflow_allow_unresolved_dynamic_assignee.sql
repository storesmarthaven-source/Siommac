-- ============================================================================
-- Atomic Workflow Creation — allow UNRESOLVED dynamic assignees (finding #3, A10 prep)
-- create-or-replace wf_internal._resolve_and_validate_assignee + wf_internal._create_instance
-- so a dynamic context-derived assignee (supervisor/department_manager/site_manager/
-- hse_manager/document_owner/permit_area_owner/record_owner/requester_manager) that does
-- NOT resolve yields an UNASSIGNED first task (assigned_to/assigned_role NULL), matching
-- the live TS engine, instead of raising WF422. Static fixed_user/role still must resolve.
-- Comment-stripped for a clean Supabase SQL Editor paste. Idempotent (create or replace).
