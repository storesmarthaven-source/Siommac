...
// A dynamic module_key/owner_role may not be one of the keys above, so the lookup is
// honestly `| undefined` — the caller's `?? DEPARTMENT_META.hr` supplies the guaranteed default.
const deptMeta = (key: string): { key: CalendarSourceDepartment; label: string } | undefined => DEPARTMENT_META[key];
...
export async function financeStatutoryDeadlines(ctx: AdapterContext): Promise<CalendarItemDTO[]> {
  ...
  return data.map((r: RemittanceRow) => {
    ...
    return deadline({
      ...
      notes:        `${r.currency} ${r.total_due.toLocaleString('en-US', { minimumFractionDigits: 2 })} due to ${label}.`,
    });
  });
}
...
export async function hrOnboardingDeadlines(ctx: AdapterContext): Promise<CalendarItemDTO[]> {
  ...
  return data.map((t: OnboardingTaskRow) => {
    ...
    const department = deptMeta(t.module_key ?? '') ?? deptMeta(t.owner_role ?? '') ?? DEPARTMENT_META.hr;
