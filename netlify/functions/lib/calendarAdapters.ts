...
import { sb } from './db';
import type { CalendarItemDTO, CalendarTaskStatus } from '../../../types/calendar';
...
interface OnboardingTaskRow {
  ...
  status: string; due_at: string | null; case_id: string;
}
...
export async function hrOnboardingDeadlines(ctx: AdapterContext): Promise<CalendarItemDTO[]> {
  ...
  let q = sb
    ...
    .select('id, task_title, assigned_to, status, due_at, case_id')
    .not('due_at', 'is', null)
  ...
  return (data ?? []).map((t: OnboardingTaskRow) => deadline({
