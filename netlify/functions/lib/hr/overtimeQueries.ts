...
export async function listOvertimeReport(opts: { employeeId?: string; status?: string } = {}): Promise<OvertimeReportRow[]> {
  let q = sb.from('hr_overtime_entries')
    .select('id, overtime_no, employee_id, work_date, hours, multiplier, status, approved_by, approved_at, created_at')
    .order('work_date', { ascending: false });
  if (opts.employeeId) q = q.eq('employee_id', opts.employeeId);
  if (opts.status) q = q.eq('status', opts.status);
  const { data, error } = await q;
  if (error) throw Object.assign(new Error('listOvertimeReport: ' + error.message), { status: 500 });
  return ((data ?? []) as { id: string; overtime_no: string | null; employee_id: string; work_date: string; hours: number; multiplier: number; status: string; approved_by: string | null; approved_at: string | null; created_at: string }[]).map(r => ({
    id: r.id,
