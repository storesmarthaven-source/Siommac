...
// ─────────────────────────────────────────────────────────────────────────────
// Row shapes for the list reads (support_tickets / ticket_replies / app_users).
// These name the exact columns each .select() requests so the mapped response
// contract is type-checked instead of flowing through `any`.
// ─────────────────────────────────────────────────────────────────────────────
...
interface AccountSupportListRow {
  id:                        string;
  from_username:             string;
  from_name:                 string | null;
  ticket_number:             string;
...
async function _genTicketNumber(): Promise<string> {
  ...
  if (maxRow?.ticket_number) {
    const m = /(\d+)$/.exec(maxRow.ticket_number);
...
async function _insertAccountSupportTicket(
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    ...
    ticketNumber = 'TKT-' + String(Date.now()).slice(-6) + String(Math.floor(Math.random() * 100));
  }
...
> {
  ...
  if (config.ownership_model === 'dedicated_team' && config.assigned_user_id) {
    ...
    assigneeActive = assignee?.status === 'active';
  }
...
router.post('/createTicket', async c => {
  ...
  const category = ((c.get('body').args ?? {}) as Partial<Record<string, string>>).category?.trim() ?? 'general';
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    ({ data, error } = await sb.from('support_tickets').insert({
      from_user_id: actor.id, from_username: actor.username, from_name: actor.full_name,
...
router.post('/getTickets', async c => {
  ...
  const columns = 'id, from_username, from_name, ticket_number, category, subject, body, status, created_at, updated_at, cleared_by_admin, cleared_by_employee';
  const fallbackColumns = 'id, from_username, from_name, ticket_number, category, subject, body, status, created_at, updated_at';

  const primaryQuery = canViewAll
    ? sb.from('support_tickets').select(columns)
  ...
  let { data: tickets, error } = await primaryQuery.overrideTypes<TicketListRow[], { merge: false }>();
  if (error && (error.message.includes('cleared_by_admin') || error.message.includes('cleared_by_employee'))) {
    ...
    let fallbackQ = sb.from('support_tickets').select(fallbackColumns);
    if (!canViewAll) fallbackQ = fallbackQ.eq('from_user_id', actor.id);
    ({ data: tickets, error } = await fallbackQ
      .order('created_at', { ascending: false })
      .limit(canViewAll ? 50 : 20)
      .overrideTypes<TicketListRow[], { merge: false }>());
  }
  if (error) return c.json({ success: false, message: error.message });

  const ticketRows = tickets ?? [];
  const ids = ticketRows.map(t => t.id);
  let replies: TicketReplyRow[] = [];
  if (ids.length) {
    const { data: replyRows } = await sb.from('ticket_replies')
      .select('id, ticket_id, from_username, from_name, body, is_internal, created_at')
      .in('ticket_id', ids)
      .order('created_at', { ascending: true })
      .overrideTypes<TicketReplyRow[], { merge: false }>();
    replies = replyRows ?? [];
  }

  // Internal notes are only visible to staff who hold tickets.reply_internal
  const visibleReplies = replies.filter(r => canReplyInternal || !r.is_internal);

  const ticketUsernames = [...new Set([
    ...ticketRows.map(t => t.from_username),
    ...visibleReplies.map(r => r.from_username),
  ].filter(Boolean))];
  const ticketPhotoMap = new Map<string, string>();
  if (ticketUsernames.length) {
    const { data: photoUsers } = await sb.from('app_users')
      .select('id, username, profile_image')
      .in('username', ticketUsernames)
      .overrideTypes<PhotoUserRow[], { merge: false }>();
    await Promise.all((photoUsers ?? []).map(async u => {
      ticketPhotoMap.set(u.username, await getProfileSignedUrl(u.id, u.profile_image).catch(() => ''));
    }));
  }

  const replyMap = new Map<string, MappedReply[]>();
  for (const r of visibleReplies) {
    ...
    arr.push({
      ...
      fromPhoto: ticketPhotoMap.get(r.from_username) ?? '', body: r.body,
  ...
  return c.json({
    success: true,
    openCount: ticketRows.filter(t => t.status === 'open').length,
    data: ticketRows.map(t => ({
      id: t.id, fromUsername: t.from_username, fromName: t.from_name,
      fromPhoto: ticketPhotoMap.get(t.from_username) ?? '',
...
router.post('/replyTicket', async c => {
  ...
  const { error } = await sb.from('ticket_replies').insert({
    ...
    from_name: actor.full_name, body, is_internal: isInternal ?? false,
  });
  if (error) return c.json({ success: false, message: error.message });
...
router.post('/updateTicketStatus', async c => {
  ...
  if (wasOpen && (status === 'closed' || status === 'resolved')) {
    const systemMsg = status === 'closed'
      ? `This ticket has been closed by ${actor.full_name}. No further replies are possible.`
      : `This ticket has been marked as resolved by ${actor.full_name}. If your issue persists, please open a new ticket.`;
...
router.post('/deleteTicket', async c => {
  ...
  if (ticket.status === 'open' || ticket.status === 'in_progress') {
    await sb.from('ticket_replies').insert({
      ...
      body: `This ticket was deleted by the employee (${actor.full_name}) before it was resolved.`,
      is_internal: false,
    });
  }
...
router.post('/clearClosedTickets', async c => {
  ...
  const { data: toHide, error: fetchErr } = await query.overrideTypes<{ id: string }[], { merge: false }>();
  if (fetchErr) return c.json({ success: false, message: fetchErr.message });
  if (!toHide.length) return c.json({ success: true, count: 0 });
...
router.post('/createAccountSupportRequest', async c => {
  ...
  const explicitRecipients: { userId: string; reason: 'actor' | 'participant' | 'assignee' }[] = [
  ...
  if (destination.assignedUserId && destination.assignedUserId !== actor.id) {
    explicitRecipients.push({ userId: destination.assignedUserId, reason: 'assignee' });
  }
...
router.post('/getAccountSupportRequests', async c => {
  ...
  const { data: tickets, error } = await query.overrideTypes<AccountSupportListRow[], { merge: false }>();
  if (error) return c.json({ success: false, message: error.message });
...
router.post('/requestAccountAssistance', async c => {
  ...
  const explicitRecipients: { userId: string; reason: 'actor' | 'participant' }[] = [
    { userId: actor.id,   reason: 'actor' },
...
router.post('/restoreAccount', async c => {
  ...
  const evtResult = await emitAppEvent({
    ...
    notification: {
      ...
      body: `Your account has been restored by ${actor.full_name}.`,
      type: 'system_announcement',
    },
    explicitRecipients: [{ userId: subjectId, reason: 'participant' }],
  });
...
router.post('/updateAccountSupportConfig', async c => {
  ...
  const { error: upsertError } = existing
    ? await sb.from('org_account_support_config').update({
    ...
    : await sb.from('org_account_support_config').insert({
        ownership_model:   ownershipModel,
        capability_target: capabilityTarget ?? null,
        assigned_user_id:  assignedUserId ?? null,
        notes:             notes ?? null,
