...
router.post('/weather/snapshot', async c => {
  ...
  try {
    ...
    const dates = (daily['time'] ?? []) as unknown[];
    ...
    const hourTimes = (hourly['time'] ?? []) as unknown[];
    const nowHour = typeof cur['time'] === 'string' ? (cur['time'] as string).slice(0, 13) : null;
    ...
    const snapshot: WeatherSnapshot = {
      ...
      daily: days,
      hourly: hours,
      observedAt: typeof cur['time'] === 'string' ? cur['time'] as string : new Date().toISOString(),
    };
