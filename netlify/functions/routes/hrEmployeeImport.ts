...
function applyMapping(raw: Record<string, string>, mapping: Record<string, string>): Record<string, string> {
  ...
  for (const [field, col] of Object.entries(mapping)) {
    if (col && raw[col] !== undefined) out[field] = norm(raw[col]);
  }
  return out;
}
...
function deriveUsername(m: Record<string, string>): string {
  ...
  if (m['email'] && m['email'].includes('@')) return m['email'].split('@')[0]!.toLowerCase();
...
function validateRow(m: Record<string, string>, importMode: string, policy: ImportPolicy, ctx: LookupCtx): RowVerdict {
  ...
  if (numDup) {
    if (importMode === 'update' || importMode === 'create_update' || policy.duplicateEmployeeNumber === 'update') {
...
async function loadBatch(id: string) {
  const { data } = await sb.from('hr_employee_import_batches').select('*').eq('id', id).maybeSingle<Record<string, unknown>>();
  return data;
}
...
router.post('/employees/import/map-fields', async c => {
  ...
  const v = zv(c, z.object({ batchId: z.string().uuid(), mapping: z.record(z.string(), z.string()) }), body(c));
  if (!v.ok) return v.response;
  const { error } = await sb.from('hr_employee_import_batches').update({ mapping: v.data.mapping, status: 'mapped' }).eq('id', v.data.batchId);
...
router.post('/employees/import/set-policy', async c => {
  const actor = await requirePermission(c, 'hr.employees.import.map');
  const v = zv(c, z.object({ batchId: z.string().uuid(), policy: PolicySchema }), body(c));
  if (!v.ok) return v.response;
...
router.post('/employees/import/validate', async c => {
  ...
  const v = zv(c, z.object({ batchId: z.string().uuid() }), body(c));
  if (!v.ok) return v.response;
  ...
  const importMode = String(batch['import_mode'] ?? 'create');
...
router.post('/employees/import/resolve-row', async c => {
  const actor = await requirePermission(c, 'hr.employees.import.validate');
  const v = zv(c, z.object({
    batchId: z.string().uuid(), rowId: z.string().uuid(),
...
function toProvisionInput(m: Record<string, string>, policy: ImportPolicy, ctx: LookupCtx, batch: Record<string, unknown>) {
  ...
  return {
    identity: {
      ...
      password: policy.createLogins ? crypto.randomBytes(12).toString('base64url') : undefined,
...
router.post('/employees/import/commit', async c => {
  ...
  const batch = await loadBatch(v.data.batchId);
  if (!batch) return c.json({ success: false, message: 'Import batch not found.' }, 404 as 200);
  if (batch['status'] !== 'validated') return c.json({ success: false, message: `Validate the batch first (status: ${batch['status']}).` }, 400 as 200);
  const policy: ImportPolicy = { ...DEFAULT_POLICY, ...((batch['policy'] ?? {}) as Partial<ImportPolicy>) };
...
router.post('/employees/import/report', async c => {
  await requirePermission(c, 'hr.employees.import.report.download');
  const v = zv(c, z.object({ batchId: z.string().uuid() }), body(c));
  if (!v.ok) return v.response;
