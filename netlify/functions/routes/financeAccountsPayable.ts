...
import {
  ...
  createVendor, createBill, submitBill, approveBill, rejectBill, recordPayment, voidBill,
  type ApBillStatus,
...
router.post('/ap/vendors/list', async c => {
  await requirePermission(c, 'finance.ap.view');
  const v = zv(c, z.object({ status: z.enum(['active', 'inactive']).optional() }), b(c));
  if (!v.ok) return v.response;
...
router.post('/ap/vendors/create', async c => {
  ...
  const v = zv(c, z.object({ name: z.string().min(1).max(200), registrationNo: z.string().optional(), contactName: z.string().optional(), contactEmail: z.string().email().optional(), contactPhone: z.string().optional(), paymentTermsDays: z.number().int().min(0).max(365).optional(), defaultGlAccountCode: z.string().optional() }), b(c));
  if (!v.ok) return v.response;
...
router.post('/ap/bills/create', async c => {
  ...
  const v = zv(c, z.object({
    ...
    lines: z.array(z.object({ description: z.string().min(1).max(300), amount: z.number().positive(), glAccountCode: z.string().optional(), costCenterId: z.string().uuid().nullable().optional() })).min(1, 'At least one line is required.'),
  }), b(c));
  if (!v.ok) return v.response;
...
router.post('/ap/bills/approve', async c => {
  const actor = await requirePermission(c, 'finance.ap.approve');
  const v = zv(c, z.object({ id: z.string().uuid() }), b(c));
  if (!v.ok) return v.response;
...
router.post('/ap/bills/record-payment', async c => {
  const actor = await requirePermission(c, 'finance.ap.manage');
  const v = zv(c, z.object({ id: z.string().uuid(), amount: z.number().positive(), method: z.enum(['eft', 'cheque', 'cash', 'card']).optional(), reference: z.string().max(120).optional() }), b(c));
  if (!v.ok) return v.response;
