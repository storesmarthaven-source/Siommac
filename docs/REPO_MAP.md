...
- **`ui/`** — shared component kit (`components/`, `charts/`, `layouts/`, `status/`, `toast/`, `theme/`) + `PAGE_GUIDE.md` (mandatory page structure). **`ui/widgets/`** = the widget system (see below). Standards worth knowing: `KpiTile` (`.ui-kpi*`, the plain KPI card — vs `StatsCard`, the rich chart card); `Drawer rich`/`.ui-rdrawer` (the standard navy detail-drawer shell + optional pinned footer; navy = future dark mode); `Menu` (portals to `<body>`, Lucide icons) + `PanelTabs` (primary tabs + More▾); `DataTable` (register table; row actions = one `⋮` + portalled dropdown).
...
- **Board:** `WidgetBoard` → `WidgetBoardZone` (**react-grid-layout** = layout-as-data, Preact-native; NOT gridstack — that bridge was removed) → `WidgetFrame` → `WidgetRenderer` (permission-gates on mount). `useBoardLayout.ts` persists per-user to `ui_layout.layout` (jsonb) and exposes `isDefaultDirty` (current vs org default; `getInstanceLayout` returns the raw org `default`).
...
- **Per-board config:** `WidgetBoard` accepts `cellHeight` + `column`, plus `resizable`, and `maxRows` + `isBounded` (set both to lock a single-row grid to HORIZONTAL-only movement — how the Statutory KPI strip reorders). On the board today: Employee Master, HR Onboarding, and the **Finance Statutory Configuration** dashboard (its own board `finance.statutory.v2` + a separate locked-horizontal KPI grid `finance.statutory.kpis.v2`).
