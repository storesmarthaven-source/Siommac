...
function toDesignDeadline(item: CalendarItemDTO, day: Date, today: Date): DesignDeadline {
  ...
  const whenLabel = timed
    ? new Date(item.startsAt as string).toLocaleTimeString('en-GB', { hour: 'numeric', minute: '2-digit', hour12: true })
    : '11:59 PM';
  return {
    id: item.id,
    title: item.title,
    sub: item.sourceDepartmentLabel ?? item.sourceLabel ?? item.notes ?? (item.type === 'activity' ? 'Calendar event' : 'Deadline'),
    icon: item.type === 'activity' ? 'CalendarClock' as LucideName : vis.icon,
    tone: vis.tone,
...
function parseDayKey(key: string): Date { const [y, m, d] = key.split('-').map(Number); return new Date(y ?? 1970, (m ?? 1) - 1, d ?? 1); }
