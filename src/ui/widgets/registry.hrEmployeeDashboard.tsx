...
function HrWorkQueue({ stats }: { stats: HrDashboardStats }): VNode {
  ...
  return <KpiCard title="HR Work Queue" value={s.total} detail={`${s.urgent} urgent · oldest ${s.oldest_days ?? 0}d`} icon="fa-list-check" linkLabel="View Register" tone="amber" />;
}
...
function ReadinessRadar({ stats }: { stats: HrDashboardStats }): VNode {
  ...
  useEffect(() => {
    ...
    const chart = new Chart(canvas, {
      ...
      options: { responsive: true, maintainAspectRatio: false, animation: { duration: 720 }, elements: { line: { tension: .12 } }, plugins: { legend: { display: false }, tooltip: { callbacks: { label: context => `${context.raw ?? 0}%` } } }, scales: { r: { min: 0, max: 100, beginAtZero: true, backgroundColor: 'rgba(91,202,46,.025)', ticks: { display: false, stepSize: 20 }, grid: { color: 'rgba(196,205,201,.62)' }, angleLines: { color: 'rgba(196,205,201,.62)' }, pointLabels: { color: '#414544', font: { size: 10, weight: 500 } } } } },
    });
