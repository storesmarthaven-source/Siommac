...
import {
  ...
  StatusPill as Pill, TableSearch, TableSkeleton, useFilterDropdowns, type DataTableColumn,
} from '@ui';
...
const COLUMNS: DataTableColumn<EmailTemplateSummary>[] = [
  {
    ...
    cell: row => <div class="etl-cell-template">
      <strong>{row.name}{row.protected && <span class="etl-system">System</span>}</strong>
      {row.description && <span>{row.description}</span>}
    </div>,
  },
  { id: 'trigger', header: 'Trigger', width: 'minmax(150px, 1fr)', sortValue: row => row.triggerLabel, cell: row => row.triggerLabel },
  { id: 'status', header: 'Status', width: '130px', sortValue: row => row.status, cell: row => <TemplateStatus status={row.status} /> },
  { id: 'bindings', header: 'Bindings', align: 'center', width: '110px', sortValue: row => row.activeUsageCount, cell: row => `${row.activeUsageCount} active` },
  { id: 'owner', header: 'Owner', width: 'minmax(130px, 1fr)', sortValue: row => row.ownerLabel, cell: row => row.ownerLabel },
  { id: 'updated', header: 'Last edited', width: '130px', sortValue: row => row.updatedAt, cell: row => date(row.updatedAt) },
];
          ...
          : listQuery.isError
            ...
            : rows.length === 0
              ? <EmptyState
                ...
                actions={<Button variant="primary" onClick={() => setCreateOpen(true)}>New Template</Button>}
              />
              : view === 'table'
                ? <DataTable<EmailTemplateSummary>
                  ...
                  rowActions={row => [
                    { id: 'open', label: 'Open in Email Studio', icon: <LucideIcon name="Pencil" />, onSelect: () => setOpenId(row.id) },
                    { id: 'duplicate', label: 'Duplicate', icon: <LucideIcon name="Copy" />, onSelect: () => void duplicate(row) },
                    ...(row.protected || row.status === 'archived'
                      ? []
                      : [{ id: 'archive', label: 'Archive template…', icon: <LucideIcon name="Archive" />, tone: 'danger' as const, onSelect: () => void archive(row) }]),
                  ]}
                />
