...
function DraftRecovery({ label, savedAt, stepIndex, onResume, onDiscard }: {
  ...
  // Blank/whitespace-only labels fall back to a placeholder (an empty string is "no name",
  // so `??` would be wrong here — it would keep the blank).
  const trimmedLabel = label?.trim() ?? '';
  const displayLabel = trimmedLabel.length ? trimmedLabel : 'Unnamed draft';
  return (
...
export function EmployeeCreatePage({ onClose }: { onClose: () => void }): VNode {
  ...
  const resumeDraft = useCallback(() => {
    ...
    setForm(f => ({ ...f, ...saved }));
    setStep(d.step_index);
    setMaxStep(d.step_index);
