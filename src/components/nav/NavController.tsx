...
export function NavController(): h.JSX.Element {
  ...
  useEffect(() => {
    ...
    HDR_PAIRS.forEach(({ btn, modal }) => {
      ...
      function onHdrBtnClick(e: Event): void {
        e.stopPropagation();
        const isOpen = m!.classList.contains('open');
        closeAllModals();
        if (!isOpen) {
          // Position the modal box under the button
          ...
          // Notif modal opened
          if (btn === 'hdrNotifBtn' || btn === 'empNotifBtn' || btn === 'mgrNotifBtn') {
            callWin('_fetchNotifs');
            callWin('_notifModalOpened');
          }

          // Message modal opened
          if (btn === 'hdrMsgBtn' || btn === 'empMsgBtn' || btn === 'mgrMsgBtn') {
            setTimeout(() => {
