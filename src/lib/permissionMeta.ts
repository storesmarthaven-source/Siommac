...
export const PERMISSION_META: Record<PermissionKey, PermissionMeta> = {
  ...
  'communications.compliance_read': {
    ...
    label: 'View Compliance Conversations',
    description: 'Open the Compliance workspace and read private message threads for approved, time-boxed investigations (fully audited). This is the access that shows the Compliance shield.',
    risk: 'critical',
    requiresSuperAdmin: true,
  },
  'communications.compliance_export': {
    ...
    description: 'Export message history/evidence (PDF/JSON) for an approved compliance investigation. Layered on top of View access and separately time-boxed.',
    risk: 'critical',
    requiresSuperAdmin: true,
  },
  'communications.compliance_approve': {
    ...
    description: "Review and approve/reject other users' requests for compliance View/Export access. Does NOT grant any compliance access itself — it only lets the holder sign off on granting it to others (a scoped reviewer, without full permission management).",
    risk: 'high',
    requiresSuperAdmin: true,
  },
