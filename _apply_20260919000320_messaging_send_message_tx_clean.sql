...
begin
  if p_thread_id is null then
    raise exception 'messaging_send_message: thread_id is required' using errcode = 'MSG400';
  end if;
  if p_actor_id is null or btrim(p_actor_id) = '' then
    raise exception 'messaging_send_message: actor_id is required' using errcode = 'MSG400';
  end if;
  ...
  if v_priority_safe not in ('normal','important','urgent','action_required') then
    raise exception 'messaging_send_message: invalid priority %', v_priority_safe using errcode = 'MSG400';
  end if;
  ...
  if not found then
    raise exception 'messaging_send_message: % is not an active participant in thread %',
      p_actor_id, p_thread_id using errcode = 'MSG403';
  end if;
  ...
  if p_reply_to_post_id is not null then
    if not exists (
      ...
      raise exception 'messaging_send_message: reply target % not found in thread %',
        p_reply_to_post_id, p_thread_id using errcode = 'MSG404';
    end if;
  end if;
